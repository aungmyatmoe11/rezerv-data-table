...
- **Live:** _pending deploy — URL will be added here_
- **Playground:** `/playground` — flip attributes, read the generated JSX, watch the callbacks
- **Real usage:** `/timetable` (class timetable) · **Second dataset:** `/inventory` (server-side multi-sort, tree rows)

> **Constraint honoured.** No table/grid library is used: not TanStack Table, AG Grid, MUI DataGrid,
> Ant Design `Table`, react-data-grid, nor any virtualisation library. The engine, pagination
> controls and row windowing are hand-written in `src/lib/table`. `antd` supplies only non-table
> primitives (Button, Checkbox, Radio, Dropdown, Tooltip, Empty, Spin, Switch …) and an ESLint rule
> forbids importing `Table` / `Pagination` from `antd` and anything from `@tanstack/*`.
...
`check` = typecheck → lint → unit tests (Vitest, 84 tests) → end-to-end tests (Playwright against a
...
**Ant Design vocabulary, progressive disclosure.** The prop names are the ones a React developer
...
| `ui` — markup + CSS + antd non-table primitives | `src/lib/table/ui` | `core`, `react`, antd |
...
`columnReorder.order`). Controlled slices are read from props and never written internally; the
...
| Order column (drag) | `columnReorder` (pointer + keyboard, dnd-kit) | [/playground?columnReorder=true](/playground?columnReorder=true) |
...
| --- | --- | --- |
| Sort click, 10,000 rows, paginated (aria-sort flipped + rows painted) | **129 ms** | < 200 ms |
| Sort click, 10,000 rows, `virtual`, no pagination | **99 ms** | < 200 ms |
| Page change after sorting 10,000 rows (comparator not re-run) | **26 ms** | < 100 ms |
| Virtual scroll, 60 steps × 6 rows: frame p95 / mean / mounted `<tr>` | **17.5 ms / 16.7 ms / 20 rows** | < 32 ms p95 / < 40 rows |
...
- Column resizing and persisted layouts (the `onReorder` payload is what a consumer would persist).
