...
- **Explicit `controlled` flag** — simpler to reason about but diverges from antd; consumers
...
not sort must apply the emitted `current: 1` itself (documented, antd-identical).
